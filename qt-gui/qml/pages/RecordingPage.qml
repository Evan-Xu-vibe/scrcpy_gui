...
Flickable {
    ...
    ColumnLayout {
        ...
        Rectangle { Layout.fillWidth: true; height: 1; color: palette.midlight }
        SettingRow { title: "录制格式"; description: "与文件容器保持一致"; ComboBox { width: parent.width; enabled: Boolean(app.setting("recordEnabled")); model: ["mp4", "mkv"]; currentIndex: model.indexOf(app.setting("recordFormat")); onActivated: app.setSetting("recordFormat", currentValue) } }
        Rectangle { Layout.fillWidth: true; height: 1; color: palette.midlight }
