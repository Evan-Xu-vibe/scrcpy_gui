...
Flickable {
    ...
    ColumnLayout {
        ...
        width: parent.width
        spacing: 0
        SettingRow { title: "键盘模式"; description: "SDK 兼容性最佳，UHID 模拟物理键盘"; SelectBox { width: parent.width; model: ["SDK", "UHID", "禁用"]; currentIndex: ["sdk", "uhid", "disabled"].indexOf(app.setting("keyboardMode")); onActivated: app.setSetting("keyboardMode", ["sdk", "uhid", "disabled"][currentIndex]) } }
        Rectangle { Layout.fillWidth: true; height: 1; color: Theme.border }
        SettingRow { title: "鼠标模式"; description: "选择鼠标事件注入方式"; SelectBox { width: parent.width; model: ["SDK", "UHID", "禁用"]; currentIndex: ["sdk", "uhid", "disabled"].indexOf(app.setting("mouseMode")); onActivated: app.setSetting("mouseMode", ["sdk", "uhid", "disabled"][currentIndex]) } }
        Rectangle { Layout.fillWidth: true; height: 1; color: Theme.border }
        SettingRow { title: "手柄模式"; description: "UHID 将电脑手柄模拟为 Android 物理手柄"; SelectBox { width: parent.width; model: ["禁用", "UHID"]; currentIndex: ["disabled", "uhid"].indexOf(app.setting("gamepadMode")); onActivated: app.setSetting("gamepadMode", ["disabled", "uhid"][currentIndex]) } }
        Rectangle { Layout.fillWidth: true; height: 1; color: Theme.border }
